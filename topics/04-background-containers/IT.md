...
Nel capitolo precedente, abbiamo visto come eseguire un container in modalità interattiva tramite l'opzione `-ti`.

Mentre ora vedremo come eseguirli in modalità non interattiva e in background, vedremo come ottenere la lista di tutti i
container in esecuzione, come leggerne i logs e scopriremo infine come stopparli e come ottenere la lista di tutti i 
containers arrestati.

***

Iniziamo quindi con l'esecuzione di un piccolo [container](/sources/clock/) in modalità non interattiva. 
...
provassimo a inserire un qualunque comando, non avendo a disposizione una shell esso verrà semplicemente ignorato.

L'unica cosa che possiamo fare è premere `^C` per arrestarlo.

Non avendo mai utilizzato questa immagine prima d'ora, come già visto precedentemente con l'immagine Alpine, il sistema
...
Quando premiamo `^C` viene inviato al container un segnale di interruzione `SIGINT`. Se abbiamo avviato il container in 
modalità interattiva, con l'opzione `-ti` il segnale viene inviato al processo in foreground. Mentre se abbiamo lanciato
un container in modalità non interattiva, come in questo caso, il segnale viene inviato al processo con `PID 1`.
...
Il processo `PID 1` è speciale perché ha alcune responsabilità extra. Direttamente o indirettamente si occupa dell'avvio
di tutti gli altri processi del container. Quando il processo `PID 1` termina, tutto il resto si ferma: nelle macchine
classiche stopparlo equivale a causare un kernel panic; In un container, equivale a uccidere tutti i processi avviati al
suo interno. 

E visto che non vogliamo che questo avvenga accidentalmente è stato aggiunto questo layer di protezione extra.
...
Nei sistemi unix esiste il comando `ps` che ci permette di vedere i processi in esecuzione, analogamente Docker ci mette
...
E otterremo in output altri due diversi ID:
...
In un ambiente di produzione è molto probabile che ci siano decine, se non centinaia o persino migliaia di container in 
esecuzione. E in questi casi l'output del comando `docker ps` potrebbe risultare molto lungo e difficile da leggere. Ed
è qui che entra in gioco l'opzione `-l` che ci permette di visualizzare solamente l'ultimo container avviato.
...
Che ci permette di visualizzare solamente gli ID dei container in esecuzione:
...
salvando in un log. Per visualizzarli potremo usare il comando:
...
nel caso del nostro container ogni secondo:
...
Il secondo è quello più aggressivo, che invia immediatamente il segnale `SIGKILL` che forza l'arresto del container. 

Mentre il primo è leggermente più tollerante, invia il segnale `SIGTERM` e se il processo in esecuzione è in grado di 
...
Se volessimo infine vedere che fine hanno fatto i container che abbiamo arrestato dovremo usare il comando:
