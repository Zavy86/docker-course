...
dall'immagine base di Apline, che contiene solamente una versione lite di Linux, procedere poi con l'installazione Node,
installare poi tutte le varie dipendenze, copiare il codice e gli asset dell'applicazione e infine impostare i vari
...
**classes**, ai **layer** come al concetto di **inheritance** ed ai **containers** come a delle **instances**.
...
container è a partire da un'immagine... Sembra un po' come il dilemma: _È nato prima l'uovo o la gallina?_
...
Oppure il metodo più comune, che è quello che vi ritroverete sicuramente a utilizzare quotidianamente, è tramite il
comando `docker build`, che permette di creare un'immagine a partire da un **Dockerfile**.
...
create dalla community e quelle ospitate da terze parti (rispetto al registro Docker Hub).
...
come ad esempio database e web applications...
...
ospitate su un server privato o pubblico di terze parti.
In questo caso, l'immagine dovrà contenere anche l'url (o l'indirizzo ip) del registro al quale fa riferimento.
Ad esempio l'immagine `docker pull ghcr.io/actions/actions-runner` che si trova sul registro pubblico di Google.
...
Tramite il client di Docker, possiamo chiedere al server di interfacciarsi a un registry (ufficiale o self-hosted)
per cercare, scaricare o caricare le immagini che ci interessano.
...
Che ci indicherà che l'immagine è stata scaricata correttamente.
Come abbiamo già visto nei capitoli precedente, qualora provassimo ad avviare un container con un immagine non presente
sulla nostra macchina, Docker provvederà in automatico a scaricarla.

Quando scarichiamo un'immagine possiamo anche specificarne la versione tramite il tag, se non indichiamo nulla, Docker
utilizzerà di default la versione `latest` che dovrebbe rappresentare l'ultima versione stabile disponibile.
...
spingerci senza remore ad utilizzare il tag `latest`.
...
Alcune immagini inoltre, fortunatamente sempre di più, supportano varie architetture.
